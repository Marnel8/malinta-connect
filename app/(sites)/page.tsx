...
export default function Home() {
  ...
  const categorizeOfficials = (officials: Official[]) => {
    ...
    const councilors = officials.filter(
      (official) => official.position === "councilor"
    );
    const skChairperson = officials.find(
      (official) => official.position === "skChairperson"
    );
  ...
  const fetchOfficials = async () => {
    try {
      ...
      if (result.success && result.officials) {
        ...
        const activeOfficials = result.officials.filter(
        ...
        <div className="container px-4 md:px-6">
          <div className="max-w-4xl mx-auto bg-card rounded-xl shadow-lg overflow-hidden">
            <div className="flex flex-col md:flex-row items-center">
              ...
              <div className="p-8 md:p-12 flex flex-col gap-6 w-full">
                <div className="flex flex-col items-center md:items-start">
                  <h3 className="text-lg font-medium text-muted-foreground mb-1">
                    Registered Voters Overview
                  </h3>
                  <p className="text-muted-foreground text-sm text-center md:text-left">
                    As of the latest COMELEC data, Barangay Malinta maintains an
                    active voting population across both regular and youth
                    sectors.
                  </p>
                </div>
                <div className="grid w-full gap-6 md:grid-cols-2">
                  <div className="flex flex-col items-center md:items-start rounded-lg border border-muted bg-muted/50 p-6">
                    <span className="text-sm font-medium text-muted-foreground uppercase tracking-wide">
                      Regular Voters
                    </span>
                    <span className="font-bold text-4xl md:text-5xl tracking-tight text-primary mt-2">
                      4,634
                    </span>
                    <span className="text-xs font-medium text-muted-foreground mt-3 bg-muted px-2 py-0.5 rounded-full">
      ...
      <section className="w-full py-12 md:py-24 lg:py-32 bg-muted relative overflow-hidden">
        ...
        <div className="container px-4 md:px-6 relative">
          ...
          {officialsLoading ? (
            ...
            (() => {
              const { captain, councilors, skChairperson, barangayStaff } =
                categorizeOfficials(officials);
                          ...
                          >
                            ...
                            <CardContent className="flex items-center gap-4">
                              <div className="relative h-16 w-16 rounded-full overflow-hidden flex-shrink-0">
                                <Image
                                  src={
                                    councilor.photo || "/placeholder-user.jpg"
                                  }
                                  alt={councilor.name}
                                  fill
                                  className="object-cover"
                              ...
                              <div className="text-sm text-muted-foreground">
                                {councilor.biography
                                  ? councilor.biography.substring(0, 80) +
                                    (councilor.biography.length > 80
                                      ? "..."
                                      : "")
                                  : `Serving as ${t(
                                      "officials.councilor"
                                    ).toLowerCase()} for ${councilor.term}.`}
                              </div>
                            </CardContent>
                          </Card>
                        ))}
                        ...
                        <Card className="overflow-hidden hover:shadow-md transition-all">
                          ...
                          <CardContent className="flex items-center gap-4">
                            <div className="relative h-16 w-16 rounded-full overflow-hidden flex-shrink-0">
                              <Image
                                src={
                                  skChairperson.photo || "/placeholder-user.jpg"
                                }
                            ...
                            <div className="text-sm text-muted-foreground">
                              {skChairperson.biography
                                ? skChairperson.biography.substring(0, 80) +
                                  (skChairperson.biography.length > 80
                                    ? "..."
                                    : "")
                  ...
                  {barangayStaff.length > 0 && (
                    <div className="space-y-6">
                      <div className="text-center">
                        ...
                        <p className="text-muted-foreground">
                          Administrative personnel supporting barangay
                          operations
                        </p>
                      </div>
                        ...
                        {barangayStaff.map((staff) => (
                          ...
                          >
                            ...
                            <CardContent className="flex items-center gap-4">
                              ...
                              <div className="text-sm text-muted-foreground">
                                {staff.biography
                                  ? staff.biography.substring(0, 80) +
                                    (staff.biography.length > 80 ? "..." : "")
                                  : `Serving as ${t(
                                      `officials.${staff.position}`
                                    ).toLowerCase()} for ${staff.term}.`}
                              </div>
                            </CardContent>
                          </Card>
                        ))}
