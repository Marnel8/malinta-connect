...
if (
	...
	console.error(
		"Cloudinary configuration incomplete. Missing environment variables:",
		{
...
export const uploadToCloudinary = async (
	...
	try {
		...
		if (Buffer.isBuffer(file)) {
			...
			// For file path strings or data URLs
		...
		if (error instanceof Error) {
			if (error.message.includes("Cloudinary is not properly configured")) {
				throw new Error(
					"Cloudinary configuration error: Please check your environment variables (CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET)"
				);
			}
			if (error.message.includes("Invalid file format")) {
				throw new Error(
					"Invalid file format. Please ensure the image is in JPG, PNG, or WebP format."
				);
			}
			if (error.message.includes("File too large")) {
				throw new Error(
					"File size too large. Please compress the image or use a smaller file."
				);
			}
		}
...
export const imageTransformations = {
	...
	responsive: (maxWidth: number = 1200) => `w_auto,c_scale,w_${maxWidth}`,

	avatar: (size: number = 100) => `w_${size},h_${size},c_fill,g_face,r_max`,
