...
// Read EJS template file with robust path resolution for production
const readTemplate = (templateName: string): string => {
  ...
  const possiblePaths = [
    // Strategy 1: From process.cwd() (works in dev and most production setups)
    path.join(cwd, "mails", "templates", fileName),
    // Strategy 2: Absolute path from /var/task (AWS Lambda/Vercel serverless)
    path.join("/var/task", "mails", "templates", fileName),
    // Strategy 3: From project root (Vercel)
    path.join("/var/task", fileName),
    // Strategy 4: From .next/server directory (Next.js production build)
    path.join(cwd, ".next", "server", "mails", "templates", fileName),
    // Strategy 5: From .next/server/chunks (Next.js production chunks)
    path.join(cwd, ".next", "server", "chunks", "mails", "templates", fileName),
    // Strategy 6: Relative to project root (absolute resolve)
    path.resolve(cwd, "mails", "templates", fileName),
    // Strategy 7: Try going up from .next if we're in a build directory
    cwd.includes(".next")
      ? path.join(cwd.split(".next")[0], "mails", "templates", fileName)
      : null,
  ...
  // If none found, log all attempted paths for debugging
  console.error(
    `Template ${templateName} not found. Attempted paths:`,
    possiblePaths
  );
  ...
  throw new Error(
    `Template ${templateName} not found. Checked ${possiblePaths.length} paths. CWD: ${cwd}`
  );
};
...
const renderTemplate = (
  ...
  html = html.replace(/<%=\s*process\.env\.NEXT_PUBLIC_APP_URL\s*%>/g, appUrl);
  ...
  html = html.replace(
    ...
    (_match, condition, truthyContent, falsyContent) => {
      try {
        ...
        const evaluator = new Function(...argNames, `return (${condition});`);
  ...
  try {
    ...
    switch (status) {
      ...
      case "readyForAppointment":
        ...
        intro =
          "Your blotter report is ready for appointment scheduling. Please contact us to set up an appointment.";
        break;
      case "additionalInfo":
